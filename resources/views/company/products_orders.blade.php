...
@php
    ...
    if (!empty($data['from'])) {
        ...
        if($data['delivery'] == 1) {
            $title = __('messages.show_products_orders') . " ( " . __('messages.only_delivery') . " )";
        }else {
            $title = __('messages.show_products_orders') . " ( " . __('messages.delivery_and_installation') . " )";
        }
    }
@endphp
...
@push('scripts')
    <script>
        ...
        $("#area_select, #toDate, #payment_select, #orderStatus, #delivery_methods_select").on("change", function() {
            $("#filter-form").submit()
        })
        ...
        <div class="statbox widget box box-shadow">
            ...
            <div class="row">
                <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                    ...
                    @endphp
                    <a href="{{ route('webview.company.salesReport2', $queryArray) }}" target="_blank" class="btn btn-primary">{{ __('messages.print') . ' ' . __('messages.sales_report') }}</a>
                </div>
            </div>
        </div>
        <div class="widget-content widget-content-area">
                    ...
                    <tbody>
                        ...
                        @foreach ($data['orders'] as $order)
                            <tr>
                                ...
                                <td class="text-center blue-color hide_col"><a href="{{ route('orders.company.details', $order->order_id) }}" ><i class="far fa-eye"></i></a></td>
                                <td class="text-center blue-color hide_col"><a href="{{ route('orders.company.invoice', $order->order_id) }}?download=1" target="_blank" ><i class="far fa-eye"></i></a></td>
                            </tr>
                            <?php $i ++ ?>
                        @endforeach
                    </tbody>
