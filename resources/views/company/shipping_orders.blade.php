...
@section('title' , __('messages.shippment_orders') )
@php
...
@push('scripts')
    ...
    <script>
        var ttle = "{{ __('messages.shippment_orders') }}",
    ...
    <script>
        var price = dTbls.column(7).data(),
            delivery = dTbls.column(8).data(),
            installation = dTbls.column(9).data(),
        ...
        $("#order-tbl tfoot").find('th').eq(9).text(`${totalInstallation} ${dinar}`);
                ...
                <table id="order-tbl" class="table table-hover non-hover" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th>{{ __('messages.shipment_number') }}</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($data['orders'] as $order)
                            <tr>
                                ...
                                <td>
                                    @if (count($order->oItemsCompany($data['company_id'])->get()) > 0)
                                    @foreach ($order->oItemsCompany($data['company_id'])->get() as $item)
                                    <span style="margin-bottom: 2px;" class="badge outline-badge-warning">{{ $item->shipment->airway_bill_number }}</span>
                                    @endforeach
                                    @endif
                                </td>
                                ...
                                @endphp
                                <td>{{ $order->shipping_cost . " " . __('messages.dinar') }}</td>
                                <td>{{ $order->oItemsCompany($data['company_id'])->sum('final_with_delivery') + $order->shipping_cost . " " . __('messages.dinar') }}</td>
