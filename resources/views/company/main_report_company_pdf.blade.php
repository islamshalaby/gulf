    ...
    <div class="invoice-box" style="max-width: 800px;margin: auto;padding: 30px;border: 1px solid #eee;box-shadow: 0 0 10px rgba(0, 0, 0, .15);font-size: 16px;line-height: 24px;font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif;color: #555;">
        <table cellpadding="0" cellspacing="0" style="width: 100%;line-height: inherit;text-align: left;">
                    ...
                    <table style="width: 100%;line-height: inherit;text-align: right;">
                        <tr>
                            <td class="title" style="padding: 5px;vertical-align: top;padding-bottom: 20px;font-size: 45px;text-align: center;line-height: 20px;color: #333;">
                                <img src="https://res.cloudinary.com/dyyeedzqi/image/upload/h_300,w_300/v1581928924/{{ $data['company']['image'] }}" style="width:100px; max-width:300px;">
                            </td>
            ...
            <tr class="item">
                ...
                <td style="padding: 5px;vertical-align: top;text-align: center;border-bottom: 1px solid #eee;">
                    @if(in_array($order->oItemCompany($data['company']->id)->status, [1, 2, 3]))
                    {{ __('messages.opened') }}
                    @elseif (in_array($order->oItemCompany($data['company']->id)->status, [4, 5, 6]))
                    {{ __('messages.closed') }}
                    @endif
                </td>
                <td style="padding: 5px;vertical-align: top;text-align: center;border-bottom: 1px solid #eee;">
                    {{ $order->oItemCompany($data['company']->id)->sum('final_with_delivery') . " " . __('messages.dinar') }}
                </td>
                <td style="padding: 5px;vertical-align: top;text-align: center;border-bottom: 1px solid #eee;">
                    {{ $order->address->area->deliveryCompany($data['company']->id)->delivery_cost . " " . __('messages.dinar') }}
                </td>
                <td style="padding: 5px;vertical-align: top;text-align: center;border-bottom: 1px solid #eee;">
                    {{ $order->oItemCompany($data['company']->id)->sum('final_with_delivery') + $order->address->area->deliveryCompany($data['company']->id)->delivery_cost . " " . __('messages.dinar') }}
                </td>
            </tr>
            <?php $i ++; ?>
            @endforeach
