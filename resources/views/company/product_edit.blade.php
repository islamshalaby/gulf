...
@push('scripts')
    <script>
        ...
        $("#main_sections").on("change", function () {
            ...
            $.ajax({
                url : "/company-panel/products/fetchsubcatsbycat/" + secId,
                type : 'GET',
                success : function (data) {
        ...
        var categoryId = $("#category").find("option:selected").val();
            ...
            $(".actions ul").find('li').eq(1).on("mouseover", "a.next1", function() {
                ...
                if (categorySelect > 0 && subCarOneSelect > 0 && yearInput.length > 0 && subCarTwoSelect > 0 && mainSectionSelect > 0 && subMainSectionSelect > 0 && titleEnInput.length > 0 && titleArInput.length > 0 && descriptionEnText.length > 0 && descriptionArText.length > 0) {
                    $(this).attr('href', '#next')
                    $(this).addClass('next2')
