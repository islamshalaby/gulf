                ...
                success : function (data) {
                    ...
                    data.forEach(function(element) {
                        ...
                        var permition = [],
                            ...
                            installationCost = "{{ __('messages.installation_cost') }}",
                            updateInstallationCost = "{{ route('products.update.installationCost') }}"
                        ...
                        permition[3] = `<a class="dropdown-item" href="#" data-toggle="modal" data-target="#exampleModal${element.id}">${installationCost}</a>
                        <div class="modal fade" id="exampleModal${element.id}" tabindex="-1" role="dialog" aria-labelledby="exampleModal${element.id}Label" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel">${elementName}</h5>
                                    </div>
                                    <div class="modal-body">
                                        <form action="${updateInstallationCost}" method="post" enctype="multipart/form-data" >    
                                            @csrf
                                            <input name="_method" type="hidden" value="PUT">
                                            <input name="product_id" type="hidden" value="${element.id}">
                                            <div class="form-group mb-4 outside">
                                                <label for="installation_cost">${installationCost}</label>
                                                <input required type="number" step="any" min="0" name="installation_cost" class="form-control" id="installation_cost" placeholder="${installationCost}" value="${element.installation_cost}" >
                                            </div>
                        ...
                        var priceBeforeOffer = element.price_before_offer,
                            ...
                            installationCost = element.installation_cost + " {{ __('messages.dinar') }}"

                        if (installationCost == 0 + " {{ __('messages.dinar') }}") {
                            installationCostNotSet = "{{ __('messages.installation_cost_not_set') }}"
                        }
                    ...
                    <tbody>
                        ...
                        @foreach ($data['products'] as $product)
                            <tr>
                                ...
                                <td>{{ $product->installation_cost != 0  ? $product->installation_cost . " " . __('messages.dinar') : __('messages.installation_cost_not_set') }}</td>
                                ...
                                <td class="hide_col">
                                    <div class="btn-group">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuReference5" style="will-change: transform;">
                                            ...
                                            @if(Auth::user()->update_data) 
                                            <a class="dropdown-item" href="#" data-toggle="modal" data-target="#exampleModal{{ $product->id }}">{{ __('messages.installation_cost') }}</a>
                                        ...
                                        <div class="modal fade" id="exampleModal{{ $product->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModal{{ $product->id }}Label" aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="exampleModalLabel">{{ App::isLocale('en') ? $product->title_en : $product->title_ar }}</h5>
                                                    </div>
                                                    <div class="modal-body">
                                                        <form action="{{ route('products.update.installationCost') }}" method="post" enctype="multipart/form-data" >    
                                                            @csrf
                                                            <input name="_method" type="hidden" value="PUT">
                                                            <input name="product_id" type="hidden" value="{{ $product->id }}">
                                                            <div class="form-group mb-4 outside">
                                                                <label for="installation_cost">{{ __('messages.installation_cost') }}</label>
                                                                <input required type="number" step="any" min="0" name="installation_cost" class="form-control" id="installation_cost" placeholder="{{ __('messages.installation_cost') }}" value="{{ $product->installation_cost }}" >
                                                            </div>
