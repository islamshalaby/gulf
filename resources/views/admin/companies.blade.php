...
@section('title' , __('messages.show_companies'))
                ...
                <table id="without-print" class="table table-hover non-hover" style="width:100%">
                    ...
                    <tbody>
                        ...
                        @foreach ($data['companies'] as $company)
                            <tr>
                                <td><?=$i;?></td>
                                <td>{{ App::isLocale('en') ? $company->title_en : $company->title_ar }}</td>
                                <td class="text-center blue-color"><a href="{{ route('companies.details', $company->id) }}" ><i class="far fa-eye"></i></a></td>
                                @if(Auth::user()->update_data) 
                                    <td class="text-center blue-color" ><a href="{{ route('companies.edit', $company->id) }}" ><i class="far fa-edit"></i></a></td>
                                @endif
                                @if(Auth::user()->delete_data) 
                                    <td class="text-center blue-color" >
                                        
                                        @if(count($company->products) == 0)
                                        <a onclick='return confirm("{{ __('messages.are_you_sure') }}");' href="/admin-panel/companies/delete/{{ $company->id }}" ><i class="far fa-trash-alt"></i></a>
                                        @else
