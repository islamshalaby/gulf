...
@push('scripts')
    <script>
        ...
        // fetch sub options
        function fetch_sub_options_values(el) {
            var language = "{{ Config::get('app.locale') }}",
                url = "{{ route('prodcuts.fetchsuboptions', ':option') }}",
                url = url.replace(":option", el.value),
                select = $(this)

            $.get(url, function (data) {
                if (data.length > 0) {
                    var optionName = data[0].option.title_en,
                        propValOption = ""
                        if (language == 'ar') {
                            optionName = data[0].option.title_ar
                        }
                    if ($("#properties-items .row").find(`.sub-option${data[0].option_id}`)) {
                        $("#properties-items .row").find(`.sub-option${data[0].option_id}`).remove()
                    }
                    
                    data.forEach(function (option) {
                        var optionVal = option.value_en
                            if (language == 'ar') {
                                optionVal = option.value_ar
                            }
                        propValOption += `
                    ...
                    var propValSelect = `
                        <select size="1" id="row-1-office" data-property="${data[0].option_id}" class="form-control properties-select" name="property_value_id[]">
                            ${propValOption}
                        </select>
                        `
                    $("#properties-items >.row").append(`
                        <div class="col-sm-6 sub-option${data[0].option_id}">
                            <div class="row">
                                <div class="col-sm-2">
                                    ${optionName}
                                    <input type="hidden" value="${data[0].option_id}" name="option_id[]" />
                                    <input type="hidden" name="another_option_en[]" /> 
                                    <input type="hidden" name="another_option_ar[]" />
                                </div>
                                <div class="col-sm-10">
                                    ${propValSelect}
                                </div>
                ...
                $.ajax({
                    ...
                    success : function (data) {
                        ...
                        data.forEach(function (option) {
                            ...
                            var propValSelect = `
                            <select size="1" id="row-1-office" onchange="fetch_sub_options_values(this)" data-property="${option.id}" class="form-control properties-select" name="property_value_id[]">
                                ${propValOption}
                            </select>
                            `
                            $("#properties-items .row").append(`
        ...
        $("select#sub_category_select").on("change", function() {
            ...
            $.ajax({
                url : "/admin-panel/products/fetchsubcartwobycarone/" + subCarOne,
                type : 'GET',
                success : function (data) {
                    if (data.length > 0) {
        ...
        $("select#sub_cartwo_select").on("change", function() {
            ...
            $.ajax({
                url : "/admin-panel/products/fetchsubcartwobycartwo/" + subCarTwo,
                type : 'GET',
                success : function (data) {
                    if (data.length > 0) {
        ...
        $("#main_sections").on("change", function () {
            ...
            $.ajax({
                url : "/admin-panel/products/fetchsubcatsbycat/" + secId,
                type : 'GET',
                success : function (data) {
                    if (data.length > 0) {
        ...
        var categoryId = $("#category").find("option:selected").val();
            ...
            $(".actions ul").find('li').eq(1).on("mouseover", "a.next1", function() {
                ...
                if (categorySelect > 0 && companySelect > 0 && titleEnInput.length > 0 && titleArInput.length > 0 && descriptionEnText.length > 0 && descriptionArText.length > 0  && (nextLength == 0 || shownLength == nextLength) && weightInput.length > 0 && countrySelect.length > 0) {
                    $(this).attr('href', '#next')
                    $(this).addClass('next2')
                }else {
                    $(this).attr('href', '#')
            ...
            $(".actions ul").find('li').eq(1).on('click', function() {
                ...
                if ($(".steps ul").find('li').eq(2).hasClass('current')) {
                    ...
                    $("input[name='total_quatity']").on('keyup', function() {
                        if ( !$(this).val() ) {
                            $(this).attr('valid', "0")
                            if ($(this).next('.totalQ-required').length) {
                            ...
                            $(this).attr('valid', "1")
                            $(this).next('.totalQ-required').remove()
                        }
                    })
                    ...
                    $("input[name='remaining_quantity']").on('keyup', function() {
                        ...
                        if ( !$(this).val() || Number(remainingQ) > Number(totalQ) ) {
                            ...
                            if (Number(remainingQ) > Number(totalQ)) {
                                if ($(this).next('.remainingQLess-required').length) {
                                ...
                                if ($(this).next('.remainingQ-required').length) {
            ...
            <div class="form-group mb-4">
                ...
                @if (count($data['product']->images) > 0)
                    ...
                    <div style="position : relative" class="col-md-2 product_image">
                        ...
                        <img style="width: 100%" src="https://res.cloudinary.com/dyyeedzqi/image/upload/w_100,q_100/v1581928924/{{ $image->image }}"  />
                    </div>
                    @endforeach
                @endif
                </div>
                ...
                <div class="widget-content widget-content-area">
                    <div id="circle-basic" class="">
                        ...
                        <section>
                            ...
                            <div class="form-group">
                                ...
                                <select id="category" name="car_type_id" class="form-control">
                                    ...
                                    <option {{ $data['product']['car_type_id'] == $category->id ? 'selected' : '' }} value="{{ $category->id }}">{{ App::isLocale('en') ? $category->title_en : $category->title_ar }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="sub-cat-containers">
                                <div  data-next="{{ !empty($data['product']['sub_one_car_type_id']) ? '1' : '0' }}" style="{{ empty($data['product']['sub_one_car_type_id']) ? 'display:none' : '' }}" class="form-group {{ !empty($data['product']['sub_one_car_type_id']) ? 'cat-shown' : '' }}">
                                    <label for="sub_category_select">{{ __('messages.sub_one_car_types') }} *</label>
                                    <select {{ empty($data['product']['sub_one_car_type_id']) ? 'disabled' : '' }} required id="sub_category_select" name="sub_one_car_type_id" class="form-control">
                                        <option disabled selected>{{ __('messages.select') }}</option>
                                        @foreach ( $data['sub_one_cars'] as $subOneCar )
                                        <option {{ $data['product']['sub_one_car_type_id'] == $subOneCar->id ? 'selected' : '' }} value="{{ $subOneCar->id }}">{{ App::isLocale('en') ? $subOneCar->title_en : $subOneCar->title_ar }}</option>
                                        @endforeach
                                    </select>
                                </div>
    
                                <div data-next="{{ !empty($data['product']['sub_two_car_type_id']) ? '1' : '0' }}" style="{{ empty($data['product']['sub_two_car_type_id']) ? 'display:none' : '' }}" class="form-group {{ !empty($data['product']['sub_two_car_type_id']) ? 'cat-shown' : '' }}">
                                    <label for="sub_cartwo_select">{{ __('messages.sub_two_car_types') }} *</label>
                                    <select {{ empty($data['product']['sub_two_car_type_id']) ? 'disabled' : '' }} required id="sub_cartwo_select" name="sub_two_car_type_id" class="form-control">
                                        <option disabled selected>{{ __('messages.select') }}</option>
                                        @foreach ( $data['sub_two_cars'] as $subTwoCar )
                                        <option {{ $data['product']['sub_two_car_type_id'] == $subTwoCar->id ? 'selected' : '' }} value="{{ $subTwoCar->id }}">{{ App::isLocale('en') ? $subTwoCar->title_en : $subTwoCar->title_ar }}</option>
                                        @endforeach
                                    </select>
                                </div>
    
                                <div data-next="{{ !empty($data['product']['category_id']) ? '1' : '0' }}" style="{{ empty($data['product']['category_id']) ? 'display:none' : '' }}" class="form-group {{ !empty($data['product']['category_id']) ? 'cat-shown' : '' }}">
                                    <label for="main_sections">{{ __('messages.section') }} *</label>
                                    <select {{ empty($data['product']['category_id']) ? 'disabled' : '' }} id="main_sections" name="category_id" class="form-control">
                                        <option disabled selected>{{ __('messages.select') }}</option>
                                        @foreach ( $data['sections'] as $sec )
                                        <option {{ $data['product']['category_id'] == $sec->id ? 'selected' : '' }} value="{{ $sec->id }}">{{ App::isLocale('en') ? $sec->title_en : $sec->title_ar }}</option>
                                        @endforeach
                                    </select>
                                </div>
    
                                <div data-next="{{ !empty($data['product']['sub_category_id']) ? '1' : '0' }}" style="{{ empty($data['product']['sub_category_id']) ? 'display: none' : '' }}" class="form-group {{ !empty($data['product']['sub_category_id']) ? 'cat-shown' : '' }}">
                                    <label for="sub_main_sections">{{ __('messages.sub_section') }} *</label>
                                    <select {{ empty($data['product']['sub_category_id']) ? 'disabled' : '' }} required id="sub_main_sections" name="sub_category_id" class="form-control">
                                        <option disabled selected>{{ __('messages.select') }}</option>
                                        @foreach ( $data['sub_sections'] as $subSec )
                                        <option {{ $data['product']['sub_category_id'] == $subSec->id ? 'selected' : '' }} value="{{ $subSec->id }}">{{ App::isLocale('en') ? $subSec->title_en : $subSec->title_ar }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select id="companies_select" name="company_id" class="form-control">
                                    ...
                                    <option {{ $data['product']['company_id'] == $company->id ? 'selected' : '' }} value="{{ $company->id }}">{{ App::isLocale('en') ? $company->title_en : $company->title_ar }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select id="origin_country" name="origin_country" class="form-control">
                                    ...
                                    <option {{ $data['product']['origin_country'] == $country->country_code ? 'selected' : '' }} value="{{ $country->country_code }}">{{ $country->country_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group no-border">
                                ...
                                <select id="compatible" name="compatible[]" class="form-control tags" multiple="multiple">
                                    ...
                                    <option {{ in_array($x, $data['compatible']) ? 'selected' : '' }} value="{{ $x }}">{{ $x }}</option>
                                    @endforeach
                                </select>
                            </div>  --}}
                            ...
                            <div class="form-group mb-4">
                                ...
                                <input required type="number" step="any" min="0" name="weight" class="form-control" id="weight" placeholder="{{ __('messages.weight') }}" value="{{ $data['product']['weight'] }}" >
                            </div>
                            <div class="form-group mb-4">
                                <label for="width">{{ __('messages.width') }}</label>
                                <input type="number" step="any" min="0" name="width" class="form-control" id="weight" placeholder="{{ __('messages.width') }}" value="{{ $data['product']['width'] }}" >
                            </div>
                            <div class="form-group mb-4">
                                <label for="length">{{ __('messages.length') }}</label>
                                <input type="number" step="any" min="0" name="length" class="form-control" id="length" placeholder="{{ __('messages.length') }}" value="{{ $data['product']['length'] }}" >
                            </div>
                            <div class="form-group mb-4">
                                <label for="height">{{ __('messages.height') }}</label>
                                <input type="number" step="any" min="0" name="height" class="form-control" id="height" placeholder="{{ __('messages.height') }}" value="{{ $data['product']['height'] }}" >
                            </div>
                            <div class="form-group mb-4 english-direction" >
                                <label for="demo1">{{ __('messages.description_en') }} ( {{ __('messages.desc_must') }} )</label>
                            ...
                            <div class="form-group mb-4 arabic-direction">
                                <label for="demo2">{{ __('messages.description_ar') }}</label>
                                <textarea name="description_ar" required  class="form-control"  rows="5">{{ $data['product']['description_ar'] }}</textarea>
                            </div> 
                            <div class="form-group mb-4" >
                                <label class="new-control new-checkbox new-checkbox-text checkbox-primary">
                                  <input name="type" {{ $data['product']['type'] == 1 ? 'checked' : '' }} value="1" type="radio" class="new-control-input">
                                  <span class="new-control-indicator"></span><span class="new-chk-content">{{ __('messages.new') }}</span>
                                </label>
                                <label class="new-control new-checkbox new-checkbox-text checkbox-primary">
                                    <input name="type" {{ $data['product']['type'] == 2 ? 'checked' : '' }} value="2" type="radio" class="new-control-input">
                                    <span class="new-control-indicator"></span><span class="new-chk-content">{{ __('messages.used') }}</span>
                                </label>
                                <label class="new-control new-checkbox new-checkbox-text checkbox-primary">
                                    <input name="type" {{ $data['product']['type'] == 3 ? 'checked' : '' }} value="3" type="radio" class="new-control-input">
                            ...
                            <div style="display: {{ count($data['product']->category->options) > 0 ? '' : 'none' }}" id="properties-items" class="table table-hover" style="width:100%">
                                <div class="row">
                                    @if(count($data['product']->category->options) > 0)
                                        ...
                                        <div class="col-sm-1">{{ App::isLocale('en') ? $option->title_en : $option->title_ar }} <input type="hidden" value="{{ $option->id }}" name="option_id[]" /> <input type="hidden" name="another_option_en[]" /> <input type="hidden" name="another_option_ar[]" /></div>
                                        <div class="col-sm-5">
                                            <select size="1" id="row-1-office" onchange="fetch_sub_options_values(this)" data-property="{{ $option->id }}" class="form-control properties-select" name="property_value_id[]">
                                        ...
                                        @endforeach
                                    @endif
                                    @if (count($data['sub_options']) > 0)
                                    @foreach ($data['sub_options'] as $sub)
                                    @if (count($sub->values) > 0)
                                    <div class="col-sm-6 sub-option{{ $sub->id }}">
                                        <div class="row">
                                            <div class="col-sm-2">
                                                {{ App::isLocale('en') ? $sub->title_en : $sub->title_ar }}
                                                <input type="hidden" value="{{ $sub->id }}" name="option_id[]" />
                                                <input type="hidden" name="another_option_en[]" /> 
                                                <input type="hidden" name="another_option_ar[]" />
                                            </div>
                                            <div class="col-sm-10">
                                                <select size="1" id="row-1-office" data-property="{{ $sub->id }}" class="form-control properties-select" name="property_value_id[]">
                                                    @foreach ($sub->values as $val)
                                                    @if (in_array($val->parent_id, $data['main_values']))
                                                    <option {!! App\Helpers\AdminHelpers::GetOptionSelected($data['property_values'], $val->id, $sub->id) !!} value="{{ $val->id }}">
                                                        {{ App::isLocale('en') ? $val->value_en : $val->value_ar }}
                                                    </option>
                                                    @endif
                                                    @endforeach
                                                </select>
                        ...
                        <section>
                            ...
                            <div class="form-group mb-4" >
                                <label class="new-control new-checkbox new-checkbox-text checkbox-primary">
                                  <input name="global_shipping" {{ $data['product']['global_shipping'] == 1 ? 'checked' : '' }} value="1" type="checkbox" class="new-control-input">
