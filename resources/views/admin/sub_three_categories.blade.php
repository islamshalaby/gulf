                    ...
                    <tbody>
                        ...
                        @foreach ($data['sub_categories'] as $sub_category)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('sub_two_categories.details', $sub_category->category->id) }}">
                                        {{ App::isLocale('en') ? $sub_category->category->title_en : $sub_category->category->title_ar }}
                                    </a>
                                </td>
                                <td>
                                    <a href="{{route('cat_options.show',[3, $sub_category->id])}}">
                                        <div class="">
                                ...
                                <td class="text-center blue-color"><a href="{{ route('sub_three_categories.details', $sub_category->id) }}" ><i class="far fa-eye"></i></a></td>
                                @if(Auth::user()->update_data) 
                                    <td class="text-center blue-color" ><a href="{{ route('sub_three_categories.edit', $sub_category->id) }}" ><i class="far fa-edit"></i></a></td>
                                @endif
                                @if(Auth::user()->delete_data) 
                                    <td class="text-center blue-color" >
                                        @if(count($sub_category->adPrs) == 0 && count($sub_category->subCategories) == 0)
                                        <a onclick='return confirm("{{ __('messages.are_you_sure') }}");' href="{{ route('sub_three_categories.delete', $sub_category->id) }}" ><i class="far fa-trash-alt"></i></a>
                                        @else
