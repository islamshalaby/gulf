...
@section('title' , __('messages.show_car_types'))
        ...
        <div class="statbox widget box box-shadow">
            ...
            <div class="row">
                <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                    <h4>{{ __('messages.show_car_types') }}</h4>
                </div>
            </div>
        </div>
        <div class="widget-content widget-content-area">
            <div class="table-responsive"> 
                <table id="without-print" class="table table-hover non-hover" style="width:100%">
                    ...
                    <tbody>
                        <?php $i = 1; ?>
                        @foreach ($data['car_types'] as $type)
                            <tr>
                                <td><?=$i;?></td>
                                <td>{{ App::isLocale('en') ? $type->title_en : $type->title_ar }}</td>
                                <td class="text-center blue-color"><a href="{{ route('car_types.details', $type->id) }}" ><i class="far fa-eye"></i></a></td>
                                @if(Auth::user()->update_data) 
                                    <td class="text-center blue-color" ><a href="{{ route('car_types.edit', $type->id) }}" ><i class="far fa-edit"></i></a></td>
                                @endif
                                @if(Auth::user()->delete_data) 
                                    <td class="text-center blue-color" >
                                        @if(count($type->products) == 0)
                                        <a onclick='return confirm("{{ __('messages.are_you_sure') }}");' href="{{ route('car_types.delete', $type->id) }}" ><i class="far fa-trash-alt"></i></a>
                                        @else
