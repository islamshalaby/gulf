...
<script>
    ...
    $("#category").on("change", function() {
        ...
        $("#properties-items .row").html("")
            $.ajax({
                ...
                success : function (data) {
                    ...
                    data.forEach(function (option) {
                        ...
                        var propValSelect = `
                        <select size="1" onchange="fetch_sub_options_values(this)" id="row-1-office" data-property="${option.id}" class="form-control properties-select" name="property_value_id[]">
                            ${propValOption}
                        </select>
                        `
                        $("#properties-items .row").append(`
            ...
            $('select#sub_category_select').parent('.form-group').hide()
    ...
    $("#main_sections").on("change", function () {
        ...
        $.ajax({
            url : "/admin-panel/products/fetchsubcartwobycarthree/" + secId,
            type : 'GET',
            success : function (data) {
                if (data.length > 0) {
    ...
    $("#properties-items .row").on('click', 'select', function() {
        ...
        $(".actions ul").find('li').eq(1).on("mouseover", "a.next1", function() {
            ...
            if (image.length > 0 && categorySelect > 0 && companySelect > 0 && titleEnInput.length > 0 && titleArInput.length > 0 && descriptionEnText.length > 0 && descriptionArText.length > 0 && ($(".sub-cat-containers").find('*[data-next="1"]') && shownLength == nextLength) && weightInput.length > 0 && countrySelect.length > 0) {
                $(this).attr('href', '#next')
                $(this).addClass('next2')
            }else {
                $(this).attr('href', '#')
