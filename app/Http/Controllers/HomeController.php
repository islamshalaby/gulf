...
class HomeController extends Controller
{
    ...
    public function getAdData(Request $request) {
        ...
        for ($n = 0; $n < count($bestOffers); $n ++) {
            $product = AdProduct::where('deleted', 0)->where('id', $bestOffers[$n]['product_id'])->where('status', 1)->select('id', 'title', 'price', 'publication_date as date', 'selected as feature', 'year')->first();
            if ($product) {
