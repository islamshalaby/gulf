...
class AdProductController extends Controller
{
    ...
    public function getdetails(Request $request){
        ...
        for($i = 0 ; $i < count($related); $i++){
            ...
            // $related[$i]['favorite'] = false;
            $related[$i]['image'] = AdProductImage::where('product_id' , $related[$i]['id'])->first()['image'];
        }
        $product['ad'] = Ad::where('product_type' , 2)->inRandomOrder()->first();
    ...
    public function getproducts(Request $request){
        ...
        if ($request->lang == 'en') {
            if ($request->sub_category_level3_id != 0) {
                $data['sub_categories'] = SubFourCategory::where('deleted' , 0)->where('sub_category_id' , $request->sub_category_level3_id)->select('id' , 'image' , 'title_en as title')->get()->toArray();
            }else {
            ...
            if ($request->sub_category_level2_id != 0) {
                $data['sub_category_array'] = SubThreeCategory::where('deleted' , 0)->where('sub_category_id' , $request->sub_category_level2_id)->select('id' , 'image' , 'title_en as title')->get()->toArray();
            }else {
                $data['sub_category_array'] = SubThreeCategory::where('deleted' , 0)->whereIn('sub_category_id' , $subCategoriesTwo)->select('id' , 'image' , 'title_en as title')->get()->toArray();
            }
            
        }else {
            if ($request->sub_category_level3_id != 0) {
                $data['sub_categories'] = SubFourCategory::where('deleted' , 0)->where('sub_category_id' , $request->sub_category_level3_id)->select('id' , 'image' , 'title_ar as title')->get()->toArray();
            }else {
            ...
            if ($request->sub_category_level2_id != 0) {
                $data['sub_category_array'] = SubThreeCategory::where('deleted' , 0)->where('sub_category_id' , $request->sub_category_level2_id)->select('id' , 'image' , 'title_ar as title')->get()->toArray();
            }else {
